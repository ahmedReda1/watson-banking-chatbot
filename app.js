 ...
 *
 **/
async function checkForLookupRequests(data, callback) {
	...
	if (data.context && data.context.action && data.context.action.lookup && data.context.action.lookup != 'complete') {
		...
		if (data.context.action.lookup === LOOKUP_BALANCE) {
			...
			let responseTxtAppend = '';
			if (data.context.action.append_ticket && data.context.action.append_ticket === true) {
        ticket_id = "INC000015941858"
				responseTxtAppend += 'A ticket has been created for your issue with refrence number <b>' + ticket_id + '</b></b>';
			}

			if (responseTxtAppend != '') {
				console.log('append create ticket as output.');
				if (data.output.text) {
					data.output.text.push(responseTxtAppend);
				}
				// clear the context's action since the lookup and append was completed.
...
async function createTicket(data) {
	...
	var post_response = await send_remedy_post(data);
	// loggee to test asyncronse code
...
function send_remedy_post(data) {
